...
export default function AuditPage() {
  ...
  const heroLines = ["A $1,500 audit,", "not a sales call."];
        ...
        <Shell>
          <div className="grid grid-cols-[1.2fr_1fr] gap-20 items-end max-md:grid-cols-1">
            ...
            <motion.aside 
              ...
              className="card p-7 font-mono text-[12px] shadow-sm"
            >
        ...
        <Shell>
          ...
          <div className="grid grid-cols-2 gap-0 border-t border-rule max-md:grid-cols-1 overflow-hidden">
            ...
            >
              ...
              <ul className="list-none p-0 m-0 flex flex-col gap-3.5 mt-5">
                {["2-minute form", "One-screen benchmark vs. vertical medians", "Three flags to investigate", "Automated · no human review"].map((li, i) => (
                  <li key={i} className="body-lg text-[18px] flex gap-3"><span className="mono text-mid">·</span>{li}</li>
                ))}
              </ul>
            </motion.div>
            <motion.div 
              ...
              className="p-10 pr-0 max-md:pt-5"
            >
              <Kicker>$1,500 paid audit</Kicker>
              <ul className="list-none p-0 m-0 flex flex-col gap-3.5 mt-5">
                {[
                  "Two weeks · two senior strategists",
                  "20-page written playbook",
                  "CAC headroom model built against your P&L",
                  "90-day plan you can run yourself",
                  "90-minute readout call + recording",
                  "Credited back in full if you sign a retainer"
                ].map((li, i) => (
                   <li key={i} className="body-lg text-[18px] flex gap-3"><span className="mono text-mid">·</span>{li}</li>
                ))}
              </ul>
      ...
      <Section>
        <Shell>
          <motion.div {...fadeInUp} transition={transition} className="mb-14">
            ...
            <h2 className="h1 mt-4 max-w-[18ch]">Two weeks, four checkpoints.</h2>
          </motion.div>
          ...
          <div className="grid grid-cols-4 gap-0 border-t border-rule border-l border-rule max-md:grid-cols-1 max-md:border-l-0">
            {[
              ...
              <motion.div 
                ...
                className="p-8 border-r border-rule border-b border-rule hover:bg-paper transition-colors"
              >
      ...
      <Section>
        <Shell>
          ...
          <div className="grid grid-cols-2 gap-10 max-md:grid-cols-1">
            {[
              { 
                page: "PAGE 11 · Creative", 
                id: "P-0", 
                title: "Your creative library is 68% concentrated in three ads.",
                desc: "In the last 60 days, three ads did $91,400 of your $134,200 spend. The next-best ten ads did $28,600 combined. You have no bench.",
                fix: "Ship 8 net-new concepts against the angle matrix on page 10. Kill losers at 72 hours. Target: top-ad share under 30% by day 30."
              },
              { 
                page: "PAGE 13 · Modeling", 
                id: "P-0", 
                title: "CAC headroom: $18 per new customer.",
                desc: "Against a 68% GM and 2.1× year-one LTV, your sustainable CAC is $62 — you're at $44 today. You could absorb a $34M/yr spend profile.",
                fix: "Raise your allowable CAC to $58 for 60 days. Budget headroom modeled on page 16. Expected MER impact: 3.8× → 3.4×, worth it."
              }
            ].map((sample, i) => (
              <motion.div 
                key={i}
                ...
                viewport={{ once: true }}
                transition={{ ...transition, delay: i * 0.1 }}
                className="card p-8 shadow-sm hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-baseline mb-5">
                  <span className="mono text-mid text-[11px] tracking-[0.12em] uppercase">{sample.page}</span>
                  <span className="mono text-mid text-[11px]">{sample.id}</span>
                </div>
                <h3 className="h2 text-[28px] leading-tight mb-3">{sample.title}</h3>
                <p className="body text-[14px] text-ink-2">{sample.desc}</p>
                <div className="bg-paper border border-rule h-[120px] my-5 flex items-center justify-center grayscale opacity-60">
                  <span className="mono text-[10px]">data visualization placeholder</span>
                </div>
                <div className="border-t border-rule pt-4">
                  <Kicker>The fix</Kicker>
                  <p className="body text-[14px] mt-2 leading-relaxed">{sample.fix}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <Section id="buy">
        <Shell>
          <motion.div 
            ...
            className="card p-14 bg-ink text-bg border-ink max-md:p-8 shadow-xl"
          >
            <div className="grid grid-cols-[1.5fr_1fr] gap-14 items-center max-md:grid-cols-1">
              <div>
                ...
                <div className="flex gap-[14px] mt-8 flex-wrap">
                  <Link className="btn bg-bg text-ink hover:bg-white" href="/contact">Reserve a slot</Link>
                  <Link className="btn border border-bg/50 text-bg hover:bg-bg/10" href="#faq">See FAQ</Link>
                </div>
              </div>
              <div>
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  transition={{ ...transition, delay: 0.3 }}
                  className="font-serif text-[88px] leading-none tracking-[-0.02em] max-md:text-[64px]"
                >$1,500</motion.div>
                <div className="mono text-bg/60 text-[12px] mt-3">
                  USD · paid in advance · fully credited against retainer
                </div>
                <div className="mt-7 pt-7 border-t border-bg/25">
                  <div className="kicker text-bg/60 mb-3">Slots · May 2026</div>
                  <div className="flex gap-2 font-mono text-[13px]">
                    <span className="px-2.5 py-1 border border-bg/40 line-through opacity-40">TAKEN</span>
                    <span className="px-2.5 py-1 border border-bg/40 line-through opacity-40">TAKEN</span>
                    <motion.span 
                      animate={{ backgroundColor: ['oklch(0.68 0.14 55)', 'oklch(0.72 0.16 60)', 'oklch(0.68 0.14 55)'] }}
                      transition={{ duration: 3, repeat: Infinity }}
                      className="px-3 py-1 bg-hot text-ink font-medium"
                    >1 OPEN</motion.span>
                  </div>
                </div>
              </div>
